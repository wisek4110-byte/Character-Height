...
const DraggableObject: React.FC<DraggableObjectProps> = ({ obj, index, characterCount, latestCharacterHeight, chartAreaRef, isFaded }) => {
  ...
  return (
    <motion.div
      ...
      className={`absolute cursor-grab active:cursor-grabbing pointer-events-auto z-10 ${isFaded ? 'opacity-20 grayscale brightness-110' : 'opacity-100'}`}
            ...
            {sortedCharacters.map(char => {
              ...
              return (
                <Reorder.Item 
                  ...
                  className={`flex flex-col items-center justify-end group relative cursor-grab active:cursor-grabbing outline-none pointer-events-auto ${isSelected ? 'z-30' : 'z-20'} ${isFaded ? 'opacity-20 grayscale brightness-110' : 'opacity-100'}`}
